...
import { Play, Trash2, CheckCircle2, Plus, RotateCcw } from 'lucide-react';
import { SavedSet, Screen } from '../types';
...
export default function SetsScreen({ onGoToTrain, onResume }: SetsScreenProps) {
  const { savedSets, deleteSavedSet, setCurrentPuzzleIndex, setPuzzles, updateSavedSet } = useChessStore();
  ...
  const handleResume = (set: SavedSet) => {
    const currentActive = savedSets.find(s => s.status === 'active');
    if (currentActive && currentActive.id !== set.id) {
      updateSavedSet(currentActive.id, { status: 'paused' });
    }
    updateSavedSet(set.id, { status: 'active' });
  ...
  return (
    <div className="h-screen flex flex-col bg-teal-950 text-white">
      ...
      <div className="flex justify-between items-center mb-10">
        <h1 className="font-serif text-4xl font-bold text-brand-gold">Your Sets</h1>
        <button 
          onClick={onGoToTrain}
      ...
      {savedSets.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center max-w-md mx-auto">
          ...
          <h2 className="text-2xl font-bold text-brand-gold mb-3">No Sets Yet</h2>
          <p className="text-brand-gold mb-8">
        ...
        <div className="space-y-12">
          {activeSets.length > 0 && (
            <section>
              <h2 className="text-sm font-bold text-brand-gold uppercase tracking-widest mb-6 flex items-center gap-3">
        ...
        {setToDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <h3 className="text-xl font-bold text-brand-gold mb-2">Delete Set?</h3>
...
const SetCard: React.FC<{ set: SavedSet, onResume: () => void, onDelete: () => void }> = ({ set, onResume, onDelete }) => {
  ...
  return (
    <div className={`relative bg-bg-card rounded-2xl p-6 border transition-all hover:shadow-xl group ${
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="font-serif text-xl font-bold text-brand-gold mb-1">{set.openingDisplay}</h3>
          <p className="text-xs text-text-muted font-mono">{set.puzzles.length} Puzzles</p>
        </div>
        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <ShareButton 
            title="OpenPecker Training"
            text={`I'm training the ${set.openingDisplay} opening on OpenPecker! I've completed ${set.cyclesCompleted}/${set.targetCycles} cycles with ${set.bestAccuracy}% best accuracy.`}
            className="p-2 text-text-muted hover:text-brand-gold transition-colors rounded-lg hover:bg-white/5"
          />
